...
class BaseCase(unittest2.TestCase,SharedObject):
	...
	def test_downstream(self):
		...
		# s = 
		res = singular_pipe.graph.get_downstream_nodes([ File('/tmp/digit.txt')],strict=0,flat=0,config=dir_layout)
		print('''##### no test for nodes in get_downstream_nodes()''')
		# print(res)

		res = singular_pipe.graph.get_downstream_files([ File('/tmp/digit.txt')],strict=0,flat=1,config=dir_layout)[1:]
		# res = singular_pipe.runner.get_downstream_targets(File('/tmp/digit.txt'),strict=0,flat=0,target='all',config=dir_layout)
		expect = [
	...
	def test_upstream(self):
		...
		# res ==[]
		res = singular_pipe.graph.get_upstream_files([ File(self.DIR/'job2.simple_job.out_txt')],strict=0,flat=1)[1:]
		# [0]
		expect = [
