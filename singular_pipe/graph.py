...
if 1:
    ...
    def _get_upstream_tree( lst,strict,config):
        verbose = 0
        out = []
        for x in list_flatten(lst):
            up_nodes = _get_upstream(x, strict,config)
            up_trees = _get_upstream_tree(up_nodes, strict,config)
            print('[11]',repr(x),up_nodes) if verbose else None
            out.append( [x, up_trees])
        return out
        
    def get_upstream_tree(x,strict,config=DEFAULT_DIR_LAYOUT,flat=0):
        tree = _get_upstream_tree(x,strict,config)
        tree = list_flatten(tree) if flat else tree
        return tree
        
    def get_upstream_files(x,strict,config=DEFAULT_DIR_LAYOUT,flat=0):
        tree = _get_upstream_tree(x,strict,config)
        tree = tree_filter(tree,(File,Prefix))
        tree = list_flatten(tree) if flat else tree
        return tree

    def get_upstream_nodes(x,strict,config=DEFAULT_DIR_LAYOUT,flat=0):
        tree = _get_upstream_tree(x,strict,config)
        tree = tree_filter(tree,(Caller,))
        tree = list_flatten(tree) if flat else tree
        return tree

    def get_upstream(x,strict,config=DEFAULT_DIR_LAYOUT):
        return _get_upstream(x,strict,config)
    def _get_upstream(x, strict, config):
        this = _get_upstream
        out = []
        if isinstance( x,(Prefix,File)):
            x = file_to_node(x,strict, config)
            if x is None:
                out += []
            else:
                out += [ x ]
...
if 1:
    ...
    def get_downstream_files(x,strict,config=DEFAULT_DIR_LAYOUT,flat=0):
        tree = _get_downstream_tree(x,strict,config)
        tree = tree_filter(tree,(File,Prefix))
        tree = list_flatten(tree) if flat else tree
        return tree

    def get_downstream_nodes(x,strict,config=DEFAULT_DIR_LAYOUT,flat=0):
        tree = _get_downstream_tree(x,strict,config)
        tree = tree_filter(tree,(Caller,))
        tree = list_flatten(tree) if flat else tree
        return tree

    def get_downstream_tree(x,strict,config=DEFAULT_DIR_LAYOUT,flat=0):
        tree = _get_downstream_tree(x,strict,config)
        if flat:
            tree = list_flatten(tree)
        return tree
    def _get_downstream_tree( lst,strict,config ):
        verbose = 0
        out = []
        for x in list_flatten(lst):
            up_nodes = _get_downstream(x, strict,config)
            up_trees = _get_downstream_tree(up_nodes, strict,config)
            print('[_get_downstream_tree]',repr(x),up_nodes) if verbose else None
            out.append( [x, up_trees])
        return out

    def get_downstream(obj,strict,config=DEFAULT_DIR_LAYOUT):
        return _get_downstream(obj,strict,config)

    def _get_downstream(obj,strict,config):
        out = []
